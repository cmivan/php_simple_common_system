...
<TABLE id=table2 height=25 cellSpacing=0 cellPadding=0 width=768 border=0><!-- MSTableType="layout" -->
  ...
  <TR>
    <TD bgColor=#efefef height=25>&nbsp; <IMG src="images/list.gif" 
      width=16 heigth="16"> <!--empire.url--><A class=classlinkclass 
      href="/">首页</A>&nbsp;&gt;&nbsp;<A 
...
width=768 border=1>
  <TBODY>
  <TR>
    <TD class=shadow2 vAlign=bottom width=200>
      <DIV align=left>
      <TABLE id=table5 cellSpacing=0 cellPadding=0 width="100%" border=0><!-- MSTableType="layout" -->
        <TBODY>
        <TR>
          <TD vAlign=top bgColor=#efefef><IMG src="images/search2.gif" 
            width=16 heigth="16">&nbsp; 栏目导航</TD></TR>
        <TR>
          <TD>　</TD></TR>
        <TR>
          <TD>
            <TABLE>
              <TBODY>
<?
$sql="select * from km_article_class_b ";
$query=mysql_query($sql);
while ($row = mysql_fetch_array($query)) {   
?>                 
              <TR>
                <TD>&nbsp;<IMG src="images/class.gif" border=0>&nbsp;<A 
                  href="class.php?classid=<? echo($row["classid"]);?>"><? echo($row["classname"]);?></A></TD></TR>
<?
}
?>
            </TBODY></TABLE></TD></TR>
        ...
        <TR>
          <TD vAlign=top bgColor=#efefef><IMG src="images/search2.gif" 
            width=16 heigth="16">&nbsp; 热门文章</TD></TR>
        <TR>
          <TD vAlign=top>
 <?
$sql="select * from km_article order by clicktime desc limit 0,5";
$query=mysql_query($sql);
while ($row = mysql_fetch_array($query)) {   
?>          
...
border=0>
              <TBODY>             
              <TR>
                <TD width="75%"><A title=<? echo($row["title"]);?> 
                  href="show.php?id=<? echo($row["id"]);?> " 
                  target=_blank><? echo($row["title"]);?></A></TD></TR></TBODY></TABLE>
<?
}
?>
</TD></TR>
        ...
        <TR>
          <TD vAlign=top bgColor=#efefef><IMG src="images/search2.gif" 
            width=16 heigth="16">&nbsp; 最新文章</TD></TR>
        <TR>
          <TD vAlign=top>
 <?
$sql="select * from km_article order by time desc limit 0,5";
$query=mysql_query($sql);
while ($row = mysql_fetch_array($query)) {   
?>             
            <TABLE cellSpacing=0 cellPadding=5 width="95%" align=center 
...
$sql="select * from km_article order by id desc limit $start,$pagesize";
$query=mysql_query($sql);
while ($row = mysql_fetch_array($query)) {
?>           
...
//			}
    ...
    &nbsp; 
      ...
      name=select>
			...
			{
		?>     
      	<OPTION value="index?page=<? echo($i);?>" <? if($i==$page) echo(" selected") ?>>第 <? echo($i);?> 页</OPTION>
       <?
       }
	   ?>
